...
export default function DashboardLayout({ role = "graduate", children }) {
  ...
  const graduateLinks = [
    ...
    { to: "/graduate/clock", label: "Clock In", icon: ClockIcon },
    { to: "/graduate/timesheet", label: "Timesheet", icon: CalendarIcon },
    { to: "/graduate/reports", label: "Reports", icon: DocumentIcon },
  ];
  ...
  const NavLink = ({ link, onClick }) => {
    ...
    return (
      ...
      >
        <Icon className={`w-5 h-5 ${active ? "text-white" : "text-white/40 group-hover:text-white/70"}`} />
        {link.label}
      </Link>
    );
  };
    ...
    <>
      ...
      <div className="px-3 py-4 border-t border-white/[0.06]">
        <Link
          to="/"
          onClick={onLinkClick}
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-white/40 hover:text-white hover:bg-white/[0.06] transition-all duration-200"
        >
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0a0a0a]">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06] bg-black/50 backdrop-blur-xl lg:hidden sticky top-0 z-30">
        ...
        >
          {mobileOpen ? <XIcon className="w-5 h-5 text-white" /> : <MenuIcon className="w-5 h-5 text-white" />}
        </button>
      </header>
