...
export default function GraduateDashboard() {
  ...
  useEffect(() => {
    ...
    const status = localStorage.getItem("clockStatus") || "clocked-out";
    setTimesheet(ts);
    setReports(reps);
  ...
  const recentEntries = timesheet.slice(-5).reverse();
  ...
  const statusConfig = {
    ...
    "break-ended": { color: "bg-blue-500", label: "Break Ended" },
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="page-subtitle mt-1">Here's an overview of your activity</p>
          </div>
          <Link to="/graduate/clock" className="btn-primary">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            ...
            <div className="flex items-center gap-2 mt-1">
              <div className={`w-2.5 h-2.5 rounded-full ${statusConfig[clockStatus].color}`} />
              <span className="text-lg font-semibold text-white">{statusConfig[clockStatus].label}</span>
            </div>
          </motion.div>
          ...
          >
            ...
            <span className="stat-value text-emerald-400">{approvedReports}</span>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/graduate/timesheet" className="text-sm text-brand-red hover:underline">
                View all
              </Link>
            </div>
            ...
            {recentEntries.length === 0 ? (
              <div className="py-8 text-center text-white/40">
                ...
                <Link to="/graduate/clock" className="text-brand-red hover:underline text-sm mt-2 inline-block">
                  Clock in now
                </Link>
              </div>
            ) : (
              <div className="space-y-3">
                {recentEntries.map((entry) => {
                  const duration = entry.clockOut
                    ? ((entry.clockOut - entry.clockIn) / 3600000).toFixed(1)
                    : null;
                  return (
                    <div
                      key={entry.id}
                    ...
                    >
                      ...
                      <span
                        className={`text-sm font-medium ${
                          duration ? "text-emerald-400" : "text-amber-400"
                        }`}
                      >
                        {duration ? `${duration}h` : "In Progress"}
                      </span>
                    </div>
                  );
                })}
          ...
          >
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div>
                  <p className="text-sm font-medium text-white">View Timesheet</p>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-white/50">Weekly or monthly</p>
                </div>
              </Link>
            </div>
          </motion.div>
